...
def btcprice(update: Update, context: CallbackContext):
    t = getInfo('bitcoin','1')
    update.message.reply_text("current price of bitcoin BTC is "+"%.3f USD" % t)
    time.sleep(2)
    update.message.reply_text("press /help for the MENU")

def ethprice(update: Update, context: CallbackContext):
    t = getInfo('ethereum','1027')
    update.message.reply_text("current price of ethereum ETH is "+"%.3f USD" % t)
    time.sleep(2)
    update.message.reply_text("press /help for the MENU")

def solprice(update: Update, context: CallbackContext):
    t = getInfo('solana','5426')
    update.message.reply_text("current price of solana SOL is "+"%.3f USD" % t)
    time.sleep(2)
    update.message.reply_text("press /help for the MENU")

def adaprice(update: Update, context: CallbackContext):
    t = getInfo('cardano','2010')
    update.message.reply_text("current price of cardano ADA is "+"%.3f USD" % t)
    time.sleep(2)
    update.message.reply_text("press /help for the MENU")

def dogeprice(update: Update, context: CallbackContext):
    t = getInfo('dogecoin','74')
    update.message.reply_text("current price of dogecoin DOGE is "+"%.3f USD" % t)
    time.sleep(2)
    update.message.reply_text("press /help for the MENU")
